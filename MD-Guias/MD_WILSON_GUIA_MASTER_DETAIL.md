...
⚠️ Obrigatório: para o Gii gerar relações automaticamente, as tabelas DEVEM usar ENGINE=InnoDB e possuir FOREIGN KEY reais.
...
- Tabela simples para dropdown (1:1)
...
#### Master-Detail (Book → Chapter)

- View do Book exibe GridView de Chapters

- Botão Adicionar Capítulo passa book_id via URL

- book_id fica oculto no formulário

```php
<?= $form->field($model, 'book_id')->hiddenInput()->label(false) ?>
```

##### Esaa parte  aqui é pra fazer o dropdown no _form do chapter
```php
<?= $form->field($model, 'section_id')->dropDownList(
ArrayHelper::map(Section::find()->all(), 'id', 'name'),
['prompt' => 'Selecione a seção']
) ?>
```
...
modules:
  enabled:
    ...
    - Db:
        ...
        user: "root"
        password: ""
        cleanup: true # Garante que a BD seja limpa entre os testes
```
...
modules:
  enabled:
    # Habilita módulos essenciais para testes
    - Filesystem
    - Yii2
    - Asserts
  config:
    Yii2:
      # Especifica as partes do framework Yii2 a serem usadas no teste
      part: [orm, email, fixtures]
      # Aponta para o arquivo de configuração de teste principal do backend
      configFile: "config/test.php"
      # Garante que o banco de dados seja limpo e as fixtures recarregadas para cada teste
      cleanup: true
```
